...
class glab_wait_time_slot {
    ...
    private function set_prac_attr() {
        ...
        $prac_appts = $this->appointment_layer->get_practitioner_blocked_time($this->exp_date, $this->prac_id,true);
        $this->prac_blocked_times = array(); // $treat_time
        foreach ($prac_appts as $row) {
            $app_interval = $row['app_time'] . "&" . $row['app_end_time'];
            array_push($this->prac_blocked_times, $app_interval);
        }
        var_dump($this->prac_blocked_times);
        $this->total_prac_blocked_count = count($this->prac_blocked_times); // $booking_array_count
    }
