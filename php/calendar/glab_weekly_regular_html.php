...
class glab_weekly_regular_html {
    ...
    static function draw_calendar($args) {
        extract($args);
        $dayOfWeek=array('mon'=>'1','tue'=>'2','wed'=>'3','thu'=>'4','fri'=>'5','sat'=>'6','sun'=>'7');
        // object for layers
        ...
        $requested_day = substr($wait_request_day, 0, 3);
        $wait_start_array = self::getWaitHours($waiting_appts, $requested_day, 's'); //grab all start time of waiting schedule in requested day
        $wait_end_array = self::getWaitHours($waiting_appts, $requested_day, 'e'); //grab all end time of waiting schedule in requested day
        $wait_hours_list = self::generateHoursArray($wait_start_array, $wait_end_array); //create a list for waiting hours
	...
	<span style="width:100%;padding-top:5px;padding-bottom:5px;display:block;background:none repeat scroll 0 0 #717171;">TIME</span>
        ...
        foreach ($hours_data as $hours) :
            ...
            if ($hours_part[1] == '00') :
                ...
                $wait_hours_name = glab_convert_helper::convert_24_to_12($hours_part, true);
                ...
                if ($hours_part[0] == 6) {
                    ...
                    $asd3 = "'" . $day1[0] . "'";
                    $asd4 = "'" . $hours_name . "'";
                    $wasd = "'" . $wait_hours_name . "'";

                    if ($dayOfWeek[$requested_day] == '1' AND in_array($hours_part[0], $wait_hours_list))
                        $calendar.='onClick="showAppConfrmPage(' . $asd1 . ',' . $asd2 . ',' . $asd3 . ',' . $wasd . ');"';
                    else
                        $calendar.='onClick="Appointment_view_slider12(' . $asd1 . ',' . $asd2 . ',' . $asd3 . ',' . $asd4 . ');"';

                    $calendar .= ' style="border-bottom:.5px solid #999;height:22px;' . $background = (($dayOfWeek[$requested_day] == '1' AND in_array($hours_part[0], $wait_hours_list)) ? "background:#C8C8C8;" : "") . '" >' . self::create_week_chart($_SESSION[1], $calAppArr[1], $hours_part[0], $hours_part[1]) . '</div>';
                    $calendar .= '<div class="calendar_week_day"';
                    ...
                    $calendar .= ' onClick="Appointment_view_slider12(' . $asd1 . ',' . $asd2 . ',' . $asd3 . ',' . $asd4 . ');"';
                    $calendar .= ' style="border-bottom:.5px solid #999;height:22px;' . $background = (($dayOfWeek[$requested_day] == '2' AND in_array($hours_part[0], $wait_hours_list)) ? "background:#C8C8C8;" : "") . '" >' . self::create_week_chart($_SESSION[2], $calAppArr[2], $hours_part[0], $hours_part[1]) . '</div>';
                    ...
                    if ($dayOfWeek[$requested_day] == '3' AND in_array($hours_part[0], $wait_hours_list))
                        $calendar.='onClick="showAppConfrmPage(' . $asd1 . ',' . $asd2 . ',' . $asd3 . ',' . $wasd . ');"';
                    else
                        $calendar.='onClick="Appointment_view_slider12(' . $asd1 . ',' . $asd2 . ',' . $asd3 . ',' . $asd4 . ');"';

                    $calendar .= ' style="border-bottom:.5px solid #999;height:22px;' . $background = (($dayOfWeek[$requested_day] == '3' AND in_array($hours_part[0], $wait_hours_list)) ? "background:#C8C8C8;" : "") . '" >' . self::create_week_chart($_SESSION[3], $calAppArr[3], $hours_part[0], $hours_part[1]) . '</div>';
                    ...
                    if ($dayOfWeek[$requested_day] == '4' AND in_array($hours_part[0], $wait_hours_list))
                        $calendar.='onClick="showAppConfrmPage(' . $asd1 . ',' . $asd2 . ',' . $asd3 . ',' . $wasd . ');"';
                    else
                        $calendar.='onClick="Appointment_view_slider12(' . $asd1 . ',' . $asd2 . ',' . $asd3 . ',' . $asd4 . ');"';

                    $calendar .= ' style="border-bottom:.5px solid #999;height:22px;' . $background = (($dayOfWeek[$requested_day] == '4' AND in_array($hours_part[0], $wait_hours_list)) ? "background:#C8C8C8;" : "") . '" >' . self::create_week_chart($_SESSION[4], $calAppArr[4], $hours_part[0], $hours_part[1]) . '</div>';
                    ...
                    $asd4 = "'" . $hours_name . "'";
                    $wasd = "'" . $wait_hours_name . "'";
                    if ($dayOfWeek[$requested_day] == '5' AND in_array($hours_part[0], $wait_hours_list))
                        $calendar.='onClick="showAppConfrmPage(' . $asd1 . ',' . $asd2 . ',' . $asd3 . ',' . $wasd . ');"';
                    else
                        $calendar.='onClick="Appointment_view_slider12(' . $asd1 . ',' . $asd2 . ',' . $asd3 . ',' . $asd4 . ');"';

                    $calendar .= ' style="border-bottom:.5px solid #999;height:22px;' . $background = (($dayOfWeek[$requested_day] == '5' AND in_array($hours_part[0], $wait_hours_list)) ? "background:#C8C8C8;" : "") . '" >' . self::create_week_chart($_SESSION[5], $calAppArr[5], $hours_part[0], $hours_part[1]) . '</div>';
                    ...
                    $asd4 = "'" . $hours_name . "'";
                    $wasd = "'" . $wait_hours_name . "'";
                    if ($dayOfWeek[$requested_day] == '6' AND in_array($hours_part[0], $wait_hours_list))
                        $calendar.='onClick="showAppConfrmPage(' . $asd1 . ',' . $asd2 . ',' . $asd3 . ',' . $wasd . ');"';
                    else
                        $calendar.='onClick="Appointment_view_slider12(' . $asd1 . ',' . $asd2 . ',' . $asd3 . ',' . $asd4 . ');"';

                    $calendar .= ' style="border-bottom:.5px solid #999;height:22px;' . $background = (($dayOfWeek[$requested_day] == '6' AND in_array($hours_part[0], $wait_hours_list)) ? "background:#C8C8C8;" : "") . '" >' . self::create_week_chart($_SESSION[6], $calAppArr[6], $hours_part[0], $hours_part[1]) . '</div>';
                    ...
                    if ($dayOfWeek[$requested_day] == '7' AND in_array($hours_part[0], $wait_hours_list))
                        $calendar.='onClick="showAppConfrmPage(' . $asd1 . ',' . $asd2 . ',' . $asd3 . ',' . $wasd . ');"';
                    else
                        $calendar.='onClick="Appointment_view_slider12(' . $asd1 . ',' . $asd2 . ',' . $asd3 . ',' . $asd4 . ');"';

                    $calendar .= ' style="border-bottom:.5px solid #999;height:22px;' . $background = (($dayOfWeek[$requested_day] == '7' AND in_array($hours_part[0], $wait_hours_list)) ? "background:#C8C8C8;" : "") . '" >' . self::create_week_chart($_SESSION[7], $calAppArr[7], $hours_part[0], $hours_part[1]) . '</div>';
                    $calendar .= '</div>';
                    ...
                    if ($dayOfWeek[$requested_day] == '2' AND in_array($hours_part[0], $wait_hours_list))
                        $calendar.='onClick="showAppConfrmPage(' . $asd1 . ',' . $asd2 . ',' . $asd3 . ',' . $wasd . ');"';
                    else
                        $calendar.='onClick="Appointment_view_slider12(' . $asd1 . ',' . $asd2 . ',' . $asd3 . ',' . $asd4 . ');"';
                    ...
                    $calendar .= ' style="height:22px;' . $background = (($dayOfWeek[$requested_day] == '1' AND in_array($hours_part[0], $wait_hours_list)) ? "background:#C8C8C8;" : "") . '" >' . self::create_week_chart($_SESSION[1], $calAppArr[1], $hours_part[0], $hours_part[1]) . '</div>';
                    ...
                    $calendar.=' style="height:22px;' . $background = (($dayOfWeek[$requested_day] == '2' AND in_array($hours_part[0], $wait_hours_list)) ? "background:#C8C8C8;" : "") . '" onClick="javascript:showDailyView(' . $day2[2] . ',' . $day2[1] . ',' . $day2[0] . ');">' . self::create_week_chart($_SESSION[2], $calAppArr[2], $hours_part[0], $hours_part[1]) . '</div>';
                    ...
                    $calendar.=' style="height:22px;' . $background = (($dayOfWeek[$requested_day] == '3' AND in_array($hours_part[0], $wait_hours_list)) ? "background:#C8C8C8;" : "") . '" >' . self::create_week_chart($_SESSION[3], $calAppArr[3], $hours_part[0], $hours_part[1]) . '</div>';
                    ...
                    $calendar.=' style="height:22px;' . $background = (($dayOfWeek[$requested_day] == '4' AND in_array($hours_part[0], $wait_hours_list)) ? "background:#C8C8C8;" : "") . '" >' . self::create_week_chart($_SESSION[4], $calAppArr[4], $hours_part[0], $hours_part[1]) . '</div>';
                    ...
                    $calendar.='style="height:22px;' . $background = (($dayOfWeek[$requested_day] == '5' AND in_array($hours_part[0], $wait_hours_list)) ? "background:#C8C8C8;" : "") . '" >' . self::create_week_chart($_SESSION[5], $calAppArr[5], $hours_part[0], $hours_part[1]) . '</div>';
		...
		$calendar.=' style="height:22px;'.$background=(($dayOfWeek[$requested_day]=='6' AND in_array($hours_part[0],$wait_hours_list))?"background:#C8C8C8;":"").'" >'.self::create_week_chart($_SESSION[6],$calAppArr[6],$hours_part[0],$hours_part[1]).'</div>';
		...
		$calendar.=' style="height:22px;'.$background=(($dayOfWeek[$requested_day]=='7' AND in_array($hours_part[0],$wait_hours_list))?"background:#C8C8C8;":"").'" onClick="javascript:showDailyView('.$day7[2].','.$day7[1].','.$day7[0].');">'.self::create_week_chart($_SESSION[7],$calAppArr[7],$hours_part[0],$hours_part[1]).'</div>';
    ...
    static function getWaitHours($result, $day, $return_type) {
        $return_array = array();
        if (count($result) > 0) {
            foreach ($result as $row):
                $hours_array = explode('-', $row[$day]);
                if (strtolower($return_type) == 's')
                    $hour_part = explode(':', $hours_array[0]);
                else
                    $hour_part = explode(':', $hours_array[1]);

                $return_array[] = (int) $hour_part[0];
            endforeach;
        }
        return $return_array;
    }

    static function createWaitBar($startArr, $endArr) {
        $bar = '';
        $total_appointment = count($startArr);
        if ($total_appointment > 0) {
            $inc = 0;
            foreach ($startArr as $start) {
                $hour_diff = $endArr[$inc] - $start;
                $totalHeight = $hour_diff * 35;
                $bar.='<div align="center" class="verticalBar" style="width:90%;margin:0 auto;">
						<div align="center" ;="" onclick="javascript:showAppDetails()" class="barDim" style="display:block;left:10px;height: ' . $totalHeight . 'px;bottom:0px;background:#f51905;width:108px;">&nbsp;</div></div>';
            }
        }
        return $bar;
    }

    static function generateHoursArray($startArr, $endArr) {
        $inc = 0;
        $hours = array();
        foreach ($startArr as $start) {
            $hours = self::grabHours($start, $endArr[$inc], $hours);
            $inc++;
        }
        return $hours;
    }

    static function grabHours($startHour, $endHour, $currentArr) {
        $tmp_hour = $startHour;
        while ($tmp_hour <= $endHour) {
            if (!in_array($tmp_hour, $currentArr)) {
                array_push($currentArr, $tmp_hour);
            }
            $tmp_hour++;
        }
        return $currentArr;
